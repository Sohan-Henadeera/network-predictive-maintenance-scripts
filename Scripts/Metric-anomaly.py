...
CSV_FILE = os.path.join(SCRIPT_DIR, "..", "Data", "anomaly.csv")
INTERVAL_SECONDS = 10
INTERFACE = "eth0"
...
def add_row(rows, scan_id, ts, device_id, device_type, metric_name, value, unit, method, notes=""):
    rows.append([scan_id, ts, device_id, device_type, metric_name, value, unit, method, DEVICE_NAME, 1, notes])
...
if __name__ == "__main__":
    init_csv()
    scan_id = 0
    print(f"[{DEVICE_NAME}] ANOMALY collection every {INTERVAL_SECONDS}s. Ctrl+C to stop.")
    print("Make sure the dedicated attack device has started its ping flood / port scan before proceeding.")
    while True:
        scan_id += 1
